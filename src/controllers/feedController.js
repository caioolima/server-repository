...
exports.getFeed = async (req, res) => {
    ...
    try {
        ...
        const following = await Relationship.find({ follower_id: userId }).select('following_id');
...
exports.likeFeedImage = async (req, res) => {
  ...
  exports.checkFeedImageLikes = async (req, res) => {
    try {
      ...
      const isLikedByUser = image.likes.includes(req.params.userId);
...
exports.savePost = async (req, res) => {
  try {
    ...
    const existingSavedPost = await SavedPost.findOne({ userId, postOwnerId, imageUrl });
    ...
    return res.status(500).json({ success: false, message: 'Internal server error while saving post.' });
  }
};
...
exports.deleteSavedPost = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Saved post deleted successfully.' });
  } catch (error) {
    console.error('Error deleting saved post:', error);
    return res.status(500).json({ success: false, message: 'Internal server error while deleting saved post.' });
  }
};
...
exports.getSavedPosts = async (req, res) => {
  try {
    ...
    const populatedSavedPosts = await Promise.all(savedPosts.map(async (savedPost) => {
    ...
    return res.status(200).json({ success: true, savedPosts: populatedSavedPosts });
  } catch (error) {
    console.error('Error getting saved posts:', error);
    return res.status(500).json({ success: false, message: 'Internal server error while getting saved posts.' });
  }
};
